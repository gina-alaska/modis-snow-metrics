...
;This program is modified on 2014/4/10 by Jzhu from time_series_oneyear_dailysnow_kp.pro. It reflect immidiate results different stages of filtering process
;they are newcv_aj,   

pro stat_time_series_oneyear_dailysnow_kp,tmp_cover,tmp_fract,tmp_quali,tmp_albed,bnames,day_str,mid_cover,mid_fract,mid_bname,vmetrics,newcv_aj,newcv_sc,newcv_gl
...
;define a new mflg: ocen-1,land-2,lake-3 and no-snow=10, broken snow=20, consevutive snow=30,
;mflag is conposed as snow_flag +type flag, so no-snow ocean=11,no-snow land=12,no-snow lake=13
...
vmetrics(*)=0 ; initialize the metrics data
...
if cnt_snow EQ 0 then begin  ; <1> no any snow ponit in the time series
...
;idxnosnow = where(tmp_cover EQ  25,totnosnow)
;idxcloud  = where(tmp_cover EQ  50, totcloud)

;vmetrics[6] = cnt_snow ; num of snow days
;vmetrics[7] = totnosnow; num of no-snow days
  
vmetrics[9]= snow_flg + type_flg 
;vmetrics[10]= totcloud

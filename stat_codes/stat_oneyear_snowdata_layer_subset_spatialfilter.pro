pro stat_oneyear_snowdata_layer_subset_spatialfilter,flist_cover, flist_fract,flist_quali,flist_albed, upper_l_lon,upper_l_lat, lower_r_lon,lower_r_lat
...
;jzhu,4/21/2014, modifed from oneyear_snowdata_layer_subset.pro, output raw stacked data and spatial-filtering data.
...
flist_cover='/center/w/jzhu/nps/snow_metrics/2012/2012_flist_cover'
flist_fract='/center/w/jzhu/nps/snow_metrics/2012/2012_flist_fract'
flist_quali='/center/w/jzhu/nps/snow_metrics/2012/2012_flist_quali'
flist_albed='/center/w/jzhu/nps/snow_metrics/2012/2012_flist_albed'
...
ul=[upper_l_lon,upper_l_lat]
lr=[lower_r_lon,lower_r_lat]
...
flistcover=strarr(368) ; 366/year
flistfract=strarr(368)
flistquali=strarr(368);
flistalbed=strarr(368);
...
x={flist,fn:'abc',tmp_fn: 'abc',bname:'abc',fid:0L,dims:lonarr(5),pos:0L}
...
stat_read_snowdata_spatialfilter, wrkdir,fn_cover,fn_fract,fn_quali,fn_albed, ul, lr,rt_fid_cover,rt_fid_fract,rt_fid_quali,rt_fid_albed
...
;envi_file_mng, id=rt_fid_cover,/remove
...
;envi_file_mng, id=rt_fid_fract,/remove
...
;envi_file_mng, id=rt_fid_quali,/remove
...
;envi_file_mng, id=rt_fid_albed,/remove
...
;---- layer stacking cover,fract, and quali, seperately.  ------------------
  ...
  out_name = wrkdir+year+'_snow_cover_spatialfilter'

  out_dt = t_dt

  ; open files defined in flista.tmp_fn and save the rt_fid in the flista.fid

;  fnum=n_elements(flista)
;  for k=0, fnum-1 do begin
;   envi_open_data_file,flista[k].tmp_fn,r_fid=tmp_fid
;    if (tmp_fid NE -1) then begin
;     flista[k].fid=tmp_fid    
;    endif
;  endfor   
...
;---- stack fract together
 ...
 ;fist file id is flist[0].fid
  ...
  out_name = wrkdir+year+'_snow_fract_spatialfilter'

  out_dt = d_dt

;--- open files defined in flist
; fnum=n_elements(flistq)
;  for k=0, fnum-1 do begin
;   envi_open_data_file,flistq[k].tmp_fn,r_fid=tmp_fid
;    if (tmp_fid NE -1) then begin
;     flistq[k].fid=tmp_fid    
;    endif
;  endfor   
...
;---- stack quali together
 ...
 ;fist file id is flistt[0].fid
  ...
  out_name = wrkdir+year+'_snow_quali_spatialfilter'

  out_dt = q_dt
  
;--- open files defined in flistt and save rt_fid in flisttt.fn
  
;fnum=n_elements(flistt)
   
;  for k=0, fnum-1 do begin
;   envi_open_data_file,flistt[k].tmp_fn,r_fid=tmp_fid
;    if (tmp_fid NE -1) then begin
;     flistt[k].fid=tmp_fid    
;    endif
;endfor   
...
;---- stack quali together
 ...
 ;fist file id is flistt[0].fid
  ...
  out_name = wrkdir+year+'_snow_albed_spatialfilter'

  out_dt = b_dt

; open files in flistb and save the rt_fids in the flistb.fid

;fnum=n_elements(flistb)
   
;  for k=0, fnum-1 do begin
    
;   envi_open_data_file,flistb[k].tmp_fn,r_fid=tmp_fid
;    if (tmp_fid NE -1) then begin
;     flistb[k].fid=tmp_fid    

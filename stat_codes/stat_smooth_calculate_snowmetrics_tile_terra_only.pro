...
;The input are:
; four one_snowyear_daily_snow_stack files;
;the output are:
;one_snowyear_daily_snow_smoothed
;one_snowyear_daily_snow_metrics
...
;THis program is modified by Jzhu on 2014/4/10 from smooth_calculate_snowmetrics_tile_terra_only. It ouput 3D matrix of cover data in different
;filtering stages.they are 

pro stat_smooth_calculate_snowmetrics_tile_terra_only,filen_cover,filen_fract,filen_quali,filen_albed,ver,flg
;input: filen_cover,filen_fract,filen_quali,filen_albed.  Four one_snowyear_daily_snow_stack files
;outputs: a smoothed file (filen+'_smooth') and a metrics file (filen+'_metrics'),
...
;test
;terra files
;filen_cover='/center/w/jzhu/nps/snow-metrics/2010/2010_snow_cover'
;filen_fract='/center/w/jzhu/nps/snow-metrics/2010/2010_snow_fract'
;filen_quali='/center/w/jzhu/nps/snow-metrics/2010/2010_snow_quali'
;filen_albed='/center/w/jzhu/nps/snow-metrics/2010/2010_snow_albed'
;ver='v7'
...
;---snow year is named as year after the 'year' in the filen_cover
;snow_year=string(fix(year)+1,format='(I4)')

;----open smooth file and metrics file to ready to be writen.

fileoutprx=year+'_snowyear'


fileout_cover=wrkdir+fileoutprx+'_cover_'+ver

openw,unit_cover,fileout_cover,/get_lun


fileout_smooth=wrkdir+fileoutprx+'_fract_'+ver

openw,unit_smooth,fileout_smooth,/get_lun

;record aj, snowcycle,and glacial 3D data

fileout_aj=wrkdir+fileoutprx+'_aj_'+ver

openw,unit_aj,fileout_aj,/get_lun

fileout_sc=wrkdir+fileoutprx+'_sc_'+ver

openw,unit_sc,fileout_sc,/get_lun

fileout_gl=wrkdir+fileoutprx+'_gl_'+ver

openw,unit_gl,fileout_gl,/get_lun



fileout_metrics=wrkdir+fileoutprx+'_metrics_'+ver
...
;----convert bnames to day_str

day_str=bandname2day(bnames)


;---define a data buff to store the band names of the metrics, bnames_metrics=strarry(9)
bnames_metrics = ['first_snow_day','last_snow_day','first_last_snow_day_range','longest_css_first_day','longest_css_last_day','longest_css_day_range','snow_days','no_snow_days','css_segment_num','mflag','cloud_days','tot_css_days']
vmetrics=intarr(12);
...
print, 'process '+strtrim(string(i),2)+ $
       ' of total tile '+ strtrim(string(num_of_tiles-1),2) + ', sample: '+strtrim(string(j),2)

if i EQ 1155 and j EQ 2834 then begin
...
stat_time_series_oneyear_dailysnow_kp,tmp_cover,tmp_fract,tmp_quali,tmp_albed,bnames,day_str,mid_cover,mid_smooth,mid_bname,vmetrics, newcv_aj, newcv_sc,newcv_gl
...
out_aj =bytarr(sz(1),nb_smooth)
out_sc =bytarr(sz(1),nb_smooth)
out_gl =bytarr(sz(1),nb_smooth)


nb_metrics=n_elements(vmetrics)
out_metrics=intarr(sz(1),nb_metrics)

;output head info file for spatial-filtering snow cover

map_info=envi_get_map_info(fid=rt_fid_cover)
;---output cover data header
data_type=1 ; btye type for cover
envi_setup_head, fname=fileout_cover_spatialfilter, ns=ns, nl=nl, nb=nb_smooth,bnames=bnames_smooth, $
...
data_type=1 ; btye type for aj
envi_setup_head, fname=fileout_aj, ns=ns, nl=nl, nb=nb_smooth,bnames=bnames_smooth, $
    data_type=data_type, offset=0, interleave=(intlv > 1),$
    xstart=xstart+dims[1], ystart=ystart+dims[3],map_info=map_info, $
    descrip='one-year temporal filttered cover data', /write
    
    
    data_type=1 ; btye type for cover
envi_setup_head, fname=fileout_sc, ns=ns, nl=nl, nb=nb_smooth,bnames=bnames_smooth, $
    data_type=data_type, offset=0, interleave=(intlv > 1),$
    xstart=xstart+dims[1], ystart=ystart+dims[3],map_info=map_info, $
    descrip='one-year snow cycle filterred cover data', /write
    
    data_type=1 ; btye type for cover
envi_setup_head, fname=fileout_gl, ns=ns, nl=nl, nb=nb_smooth,bnames=bnames_smooth, $
    data_type=data_type, offset=0, interleave=(intlv > 1),$
    xstart=xstart+dims[1], ystart=ystart+dims[3],map_info=map_info, $
    descrip='one-year glacial filterred cover data', /write
...
writeu,unit_aj,out_aj

writeu,unit_sc,out_sc

writeu,unit_gl,out_gl

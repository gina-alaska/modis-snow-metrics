...
pro oneyear_extension_ver15,yearv,yearbq,yearbn,st_num,ed_num,days
...
compv = bytarr(st_num+numofyear+ed_num)
compbq= bytarr(st_num+numofyear+ed_num)
compbn= strarr(st_num+numofyear+ed_num)

;fill before and after data with randomly 100b to 101b for compv, 
;we assume in befor and after data period, type=20b

;tmp = byte( fix( (randomu(1, n_elements(st_num) ) )*2 )+100 )
...
;tmp = byte( fix( (randomu(1, n_elements(ed_num) ) )*2 )+100 )

FUNCTION GetCrossOver_ver15, Xref, Yref, XData, YData, bq, bpy, UP=up, DOWN=down, ALL=ALL
      ...
      ;      If(BetweenD AND BetweenR) Then PtSC=1 ELSE PtSC=0
            ...
            ;-----------for sos crossover----------------------
              ...
              IF PtSC gt 0 AND ((DSlope lt RSlope) and DOWN ) then begin  ; for sos crossover 
               ...
               ;----check xStar related two xdata[i], xdata[i+1], if either of them are not snow points, they are good, otherwise, find next non-snow points  
               
              tmpidx=where( bq NE 4b )
              
              tmp1idx =where(tmpidx GE i+1)
              
              crossidx=tmpidx(tmp1idx(0))
              
               if crossidx EQ i+1 then begin   
               ...
               ;----- crossidx- to crossidx
               
               XPts[iCount]=crossidx
               Ypts[iCount]=Yref(crossidx)
               Cpts[iCount]=PtSc
               iCount =iCount+1  
           ...
           ;----------for EOS crossover ----------------------------------
              ...
              if PtSC GT 0 AND ((DSlope gt RSlope) and UP ) THEN BEGIN
               ...
               tmp1idx =where(tmpidx LE i)
              
               crossidx=tmpidx( tmp1idx( n_elements(tmp1idx)-1 ) )
              
                if crossidx EQ i then begin   
         ...
         NPtsidx=where(YPts NE 0, NPtscnt)

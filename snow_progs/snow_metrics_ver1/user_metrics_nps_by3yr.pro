...
pro user_metrics_nps_by3yr, ndvi, v2, bq, bn, metrics_cal_threshold, out_v

;---ndvi is smoothed three-year vector,
;---v2 is orginal three-year vector,and bn is band name for ndvi
;---bq is the ndvi type od three-year vector
;---bn is the band name of three-year vector
...
yr2idx = where( strmid(bn,0,1) EQ 1,y2cnt)

midbn=bn(yr2idx)

bpy = y2cnt  ; num of band in the mid-year

intv_day = fix( strmid( midbn(1),7,3 ) )-fix(strmid( midbn(0),7,3 ) ) 

;This is the interval days between two measurement weeks. The band name format is:n-yyyy-ddd-ddd.

start_day =fix(strmid( midbn(0),7,3) ) ; this is the first date of the first measurement week
...
metrics=ComputeMetrics_by3yr_ver16(NDVI, bq, bn, wl, bpy, CurrentBand, DaysPerBand)
